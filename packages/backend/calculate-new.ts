...
export type StateGetter<T extends WithSyntaxError | never> = {
  ...
  (id: Importers, options: { tree: boolean; external?: boolean }):
    ...
    | ValueArray
    | undefined;
  (id: Importers, options: { external?: boolean }): ValueArray | undefined;
};

export function calculate<T extends WithSyntaxError | never>(
  node: SyntaxTree<T>,
  ...
  const calculateNode = (
    ...
    let result: ValueArray[] = node.children.reduce(
      (acc: ValueArray[], child) => {
        if (typeof child === "object" && "type" in child) {
          ...
          if (state) {
            if (hasArgs) {
              acc.push(calculateNode(state as SyntaxTree<T>, args));
            } else {
          ...
          acc.push([child as Exclude<typeof child, Parameter | LineBreak | T>]);
        }
        return acc;
      },
      []
    ...
    if (node.type === "select") {
      ...
      let next: SyntaxTree<T> = {
        type: null,
        children: result.reduce((acc: SyntaxTree<T>["children"], el) => {
