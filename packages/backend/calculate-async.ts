...
export const calculateAsync = async (
  ...
  getState: (
    id: FieldId
  ) => Promise<Computation | undefined> | undefined = () => undefined,
  options: {
  ...
  for (let i = 0; i < compute.length; i += 1) {
    ...
    for (let i = 0; i < next1.length; i += 1) {
      const el = next1[i];
      ...
      if (symb.isImport(el, "field")) {
        const value = await calculateAsync(
          el.fref,
          (await getState(el.fref))!,
          getState
        );
      ...
      for (let i = 0; i < next.length; i += 1) {
        ...
        if (Array.isArray(el) && typeof el[0] === "number") {
          if (isImport) {
            const arg = await getState(
              `${id}.${isImport.id}/${el[0]}` as FieldId
            );
            if (arg) {
              acc.value.push(arg);
          ...
          const value = (await getState(`${id}.${el.id}` as FieldId)) ?? [];
  ...
  const value =
    acc.isFunc && !options.returnDefaultValue
      ? acc.func
      : acc.value.reduce((acc, cur) => [...acc, ...cur], []);

  return value as Value[]; // value.length === 1 ? value[0] : value;
};
